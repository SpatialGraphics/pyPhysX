...
void bindIO(nb::module_& m) {
    ...
    nb::class_<PxInputData, PxInputStream>(m, "PxInputData")
            ...
            .def("seek", &PxInputData::seek, "offset"_a)
    ...
    nb::class_<PxOutputStream>(m, "PxOutputStream")
            .def(
                    "write",
                    [](PxOutputStream* stream, nb::ndarray<uint8_t> buffer) {
    ...
    //    nb::class_<PxDefaultFileOutputStream, PxOutputStream>(m, "PxDefaultFileOutputStream").def(nb::init<const
    //    char*>());
}